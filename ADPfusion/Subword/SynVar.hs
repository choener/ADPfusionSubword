...
instance
  (Monad mB, PrimArrayOps arr (Subword I) x, TermStream mB ps ts s is, TermStreamContext mB ps ts s x0 i0 is (Subword I) )
  => TermStream mB (ps:.IVariable 0) (TermSymbol ts (TwITblBt bo so arr c (Subword I) x mF mB r)) s (is:.Subword I) where
--{{{
  {-# Inline termStream #-}
  termStream Proxy (ts:| TW (BtITbl _ arr) bt) (us:..u) (is:.Subword (i:.j))
    = SP.flatten mk step . termStream (Proxy @ps) ts us is
    where
      {-# Inline [0] mk #-}
      mk tstate@(TState s ii ee) =
      ...
      step (TState s ii ee, k, z)
        | z >= 0 = do let l  = j-z; kl = subword k l
                      b <- bt (upperBound arr) kl
                      return $ SP.Yield (TState s (ii:.:RiSwI l) (ee:.(arr!kl,b))) (TState s ii ee, k, z-1)
        | otherwise = return SP.Done
--}}}

instance
  (Monad mB, PrimArrayOps arr (Subword I) x, TermStream mB ps ts s is, TermStreamContext mB ps ts s x0 i0 is (Subword I)) =>
  TermStream mB (ps:.IStatic 0) (TermSymbol ts (TwITblBt bo so arr c (Subword I) x mF mB r)) s (is:.Subword I) where
--{{{
  {-# Inline [1] termStream #-}
  termStream Proxy (ts:| TW (BtITbl _ arr) bt) (us:..LtSubword u) (is:.Subword (i:.j))
    = SP.mapM (\(TState s ii ee) ->
        let RiSwI l = getIndex (getIdx s) (Proxy :: PRI is (Subword I))
            lj = subword l j
        in  bt (upperBound arr) lj >>= \b -> return $ TState s (ii:.:RiSwI j) (ee:.(arr!lj,b)))
    . termStream (Proxy :: Proxy ps) ts us is
--}}}
